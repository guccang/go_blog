...
| 模块 | 功能 |
|------|------|
| `llm` | LLM 集成，接入 DeepSeek，支持流式响应 |
| `mcp` | MCP 协议支持，文件访问、Redis 访问、自定义 MCP 服务器 |
| `agent` | 智能任务编排，支持任务拆解和自动执行 |
| `skill` | 技能系统 |
...
基于大模型的智能对话界面。
- **实时交互**: 与 AI 进行自然语言对话
- **上下文理解**: 支持多轮对话，理解项目上下文
- **辅助开发**: 提供代码建议、问题解答和创意激发
...
### 3. AI 深度集成
- 接入 DeepSeek LLM 进行智能对话和分析
- 通过 MCP 协议访问本地文件系统和 Redis
- 自定义 MCP 服务器，暴露博客系统数据
- AI 可根据博客历史数据提供个性化建议
...
5. **扩展性强**: 模块化设计便于添加新功能
6. **AI 赋能**: 接入 DeepSeek，可实时分析博客数据，通过 MCP 服务器访问 Redis 和本地文件

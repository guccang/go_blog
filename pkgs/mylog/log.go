...
func initLogFile() error {
	...
	now := time.Now()
	fileName := fmt.Sprintf("app_%s.log", now.Format("2006-01-02"))
...
func rotateLogFile() error {
	...
	now := time.Now()
	oldFileName := fmt.Sprintf("app_%s.log", now.Format("2006-01-02"))
	newFileName := fmt.Sprintf("app_%s_%s.log", now.Format("2006-01-02"), now.Format("15-04-05"))
...
func writeLog(level, message string) {
	...
	if logConfig.LogToFile {
		now := time.Now()
		expectedFileName := fmt.Sprintf("app_%s.log", now.Format("2006-01-02"))

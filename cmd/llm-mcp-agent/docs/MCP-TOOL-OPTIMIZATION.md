...
**优化方向**：不是简单截断，而是**智能提取**——从前置任务结果中提取后续任务真正需要的关键字段（如 session_id、project_name、status）。

**涉及文件**：`orchestrator.go`

### 方案 4：工具定义分层加载（渐进式披露）

**原理**：LLM 初始只看到工具的名称和一句话描述（不含完整 schema），需要调用某个工具时通过虚拟工具 `describe_tool` 获取完整参数定义。

**涉及文件**：`bridge.go`, `processor.go`

### 方案 5：多工具并行执行

**现状**：LLM 一次返回多个 tool_calls 时，当前代码顺序执行。

**优化**：对无依赖关系的工具调用并行执行，总耗时从 Nx 降为 max(1x)。

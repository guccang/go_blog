...
func TestHandleAttachmentUpgradesRemoteHTTPRedirectToHTTPS(t *testing.T) {
	obsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{
			"success": true,
			"url":     "http://obs.example.com/download/app-release.apk?sig=1",
			"method":  "GET",
			"headers": map[string]string{},
		})
	}))
	...
	if got := rec.Header().Get("Location"); got != "https://obs.example.com/download/app-release.apk?sig=1" {
		t.Fatalf("expected https redirect location, got %q", got)
	}
}

func TestHandleAttachmentFallsBackToLocalForHuaweiObsDefaultDomainAPK(t *testing.T) {
	obsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(map[string]any{
			"success": true,
			"url":     "https://bucket.obs.cn-north-4.myhuaweicloud.com/app-release.apk?sig=1",
			"method":  "GET",
			"headers": map[string]string{},
		})
	}))

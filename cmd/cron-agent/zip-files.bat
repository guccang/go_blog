...
set OUTPUT=cron-agent_%TIMESTAMP%.zip
...
set BINNAME=cron-agent%EXT%

:: 清理编译产物
del cron-agent.exe 2>nul
del cron-agent 2>nul
...
:: 打包二进制 + 配置
%SEVENZIP% a -tzip "%OUTPUT%" %BINNAME% cron-agent.json publish.sh

...
// Connection cron-agent UAP 客户端连接管理
...
func NewConnection(cfg *Config, agentID string) *Connection {
	...
	// 注册cron.task.*消息处理器
	c.RegisterHandler("cron.task.create", c.handleTaskCreate)
	c.RegisterHandler("cron.task.update", c.handleTaskUpdate)
	c.RegisterHandler("cron.task.delete", c.handleTaskDelete)
	c.RegisterHandler("cron.task.list", c.handleTaskList)
	c.RegisterHandler("cron.task.status", c.handleTaskStatus)
	c.RegisterHandler("cron.task.trigger", c.handleTaskTrigger)
...
func (c *Connection) loadTasks() {
	...
	if err != nil {
		log.Printf("[cronAgent] 加载任务失败: %v", err)
		return
	}
	...
	for _, task := range tasks {
		if task.Enabled {
			if err := c.scheduler.AddTask(task); err != nil {
				log.Printf("[cronAgent] 注册任务失败 ID=%s: %v", task.ID, err)
			} else {
				enabledCount++
			}
		}
	...
	log.Printf("[cronAgent] 加载了 %d 个任务 (%d 个已启用)", len(tasks), enabledCount)
}
	...
	return []uap.ToolDef{
		{
			...
			Parameters: agentbase.MustMarshalJSON(map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"task_type":     map[string]interface{}{"type": "string", "description": "任务类型: cron_reminder=发送提醒消息, cron_query=执行查询并返回结果", "enum": []string{"cron_reminder", "cron_query"}},
					"payload":       map[string]interface{}{"type": "object", "description": "任务负载。cron_reminder 类型需要: message(提醒内容), account(用户账号), wechat_user(微信用户ID)。cron_query 类型需要: query(查询问题), account(用户账号), wechat_user(可选,有值则发送查询结果到微信)"},
...
func (c *Connection) handleToolCall(msg *uap.Message) {
	...
	if err := json.Unmarshal(msg.Payload, &payload); err != nil {
		log.Printf("[cronAgent] invalid tool_call payload: %v", err)
	...
	log.Printf("[cronAgent] tool_call from=%s tool=%s", msg.From, payload.ToolName)
...
func (c *Connection) handleToolResult(msg *uap.Message) {
	...
	if err := json.Unmarshal(msg.Payload, &payload); err != nil {
		log.Printf("[cronAgent] invalid tool_result payload: %v", err)
		return
	}
	c.pendMu.Lock()
	...
	if ok {
		...
		log.Printf("[cronAgent] tool_result requestID=%s has no pending channel", payload.RequestID)
	}
}
...
func (c *Connection) handleTaskComplete(msg *uap.Message) {
	...
	if err := json.Unmarshal(msg.Payload, &payload); err != nil {
		log.Printf("[cronAgent] invalid task_complete payload: %v", err)
		return
	}
	log.Printf("[cronAgent] task_complete taskID=%s status=%s", payload.TaskID, payload.Status)
	...
	if ok {
		...
		log.Printf("[cronAgent] task_complete taskID=%s has no pending channel", payload.TaskID)
	}
}
...
func (c *Connection) handleTaskEvent(msg *uap.Message) {
	...
	if err := json.Unmarshal(msg.Payload, &payload); err != nil {
		log.Printf("[cronAgent] invalid task_event payload: %v", err)
		return
	}
	log.Printf("[cronAgent] task_event taskID=%s", payload.TaskID)
...
func (c *Connection) handleError(msg *uap.Message) {
	...
	if err := json.Unmarshal(msg.Payload, &payload); err != nil {
		log.Printf("[cronAgent] invalid error payload: %v", err)
		return
	}
	log.Printf("[cronAgent] error from=%s code=%s msg=%s", msg.From, payload.Code, payload.Message)
...
func (c *Connection) callRemoteTool(toolName string, args map[string]interface{}, timeout time.Duration) (string, error) {
	// 注意：cron-agent 不依赖 toolCatalog，我们直接发送到目标agent
...
func (c *Connection) sendTaskAssign(targetAgentID string, taskID string, payload interface{}, timeout time.Duration) (*uap.TaskCompletePayload, error) {
	...
	log.Printf("[cronAgent] task_assign → agent=%s taskID=%s", targetAgentID, taskID)

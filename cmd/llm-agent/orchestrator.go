...
func findSuccessfulTerminalToolCall(session *TaskSession) (ToolCallRecord, bool) {
	session.mu.Lock()
	defer session.mu.Unlock()
	for i := len(session.ToolCalls) - 1; i >= 0; i-- {
		rec := session.ToolCalls[i]
		if rec.Success && isTerminalSessionTool(rec.ToolName) {
			return rec, true
		}
	}
...
func buildSubTaskHandleKey(rootID, subTaskID string) string {
	rootID = strings.TrimSpace(rootID)
	subTaskID = strings.TrimSpace(subTaskID)
	if rootID == "" {
		return subTaskID
	}
	return rootID + "/" + subTaskID
}
...
func (ds *dagScheduler) markDone(id string, result SubTaskResult) []SubTaskPlan {
	...
	delete(ds.completedSet, id)
	delete(ds.failedSet, id)
	delete(ds.asyncSet, id)
...
func (o *Orchestrator) resumeSubTask(
	...
	if rec, ok := findSuccessfulTerminalToolCall(session); ok {
		summary := buildTerminalToolSummary(rec.ToolName, rec.Result)
		fullResult := buildSubTaskResultText(summary, extractKeyToolData(session))
		if asyncInfos := detectAsyncResults(session); len(asyncInfos) > 0 {
			session.SetStatus("async")
			session.SetResult(fullResult)
			o.saveSession(session)
			log.Printf("[Resume] 子任务 %s 已存在终止型工具结果，直接恢复为 async: %s", subtask.ID, rec.ToolName)
			return SubTaskResult{
				SubTaskID:     subtask.ID,
				Title:         subtask.Title,
				Status:        "async",
		...
		session.SetResult(fullResult)
		o.saveSession(session)
		log.Printf("[Resume] 子任务 %s 已存在终止型工具结果，直接恢复为 done: %s", subtask.ID, rec.ToolName)
		return SubTaskResult{
			SubTaskID: subtask.ID,
			Title:     subtask.Title,
			Status:    "done",
	...
	case "assistant":
		if len(lastMsg.ToolCalls) > 0 {
			...
			for _, tc := range lastMsg.ToolCalls {
				...
				if err == nil && isTerminalSessionTool(originalName) {
					summary := buildTerminalToolSummary(originalName, result)
					fullResult := buildSubTaskResultText(summary, extractKeyToolData(session))
					if asyncInfos := detectAsyncResults(session); len(asyncInfos) > 0 {
						session.SetStatus("async")
						session.SetResult(fullResult)
...
func (o *Orchestrator) SteerSubTask(subtaskID, guidance string) error {
	...
	if !ok {
		var matched *SubTaskHandle
		for _, candidate := range o.activeHandles {
			if candidate.SubTaskID != subtaskID {
				continue
			}
			if matched != nil {
				o.activeHandlesMu.Unlock()
				return fmt.Errorf("subtask %s is ambiguous across multiple root tasks", subtaskID)
			}
			matched = candidate
		}
		if matched != nil {

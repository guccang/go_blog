...
| 接口 | 参数 | 返回值 |
|------|------|--------|
| `RawGetTodosByDate` | account, date | JSON(list，每项含 id/content/done) |
| `RawGetExerciseByDate` | account, date | JSON(list，每项含 id/name/duration 等) |
...
current_date = call_tool("RawCurrentDate", {})  # 返回 str，如 "2026-03-15"
# 一次性获取整周待办数据
todos = call_tool("RawGetTodosRange", {"account": "xxx", "startDate": "2026-03-09", "endDate": current_date})
...
- 先获取当前日期（RawCurrentDate 返回 str），再计算查询范围
- 返回值标注为 str 的工具返回字符串，标注为 JSON 的返回 dict/list，直接使用即可

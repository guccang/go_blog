...
summary: AcpStartSession 传入用户原文，严禁修改
...
如果用户消息同时包含编码和其他操作（如部署、查询），必须先通过 plan_and_execute 拆解，再按本规则处理编码子任务。
## 消息传递规则

将用户的原始消息原文直接作为 prompt 传给 AcpStartSession，**严禁修改、缩写、翻译、重新措辞或添加额外内容**。
...
- 编译/语法错误修复：部署阶段发现编译错误时，通过 AcpSendMessage 让编码 agent 修复，禁止用 ExecuteCode（Python）直接改写 Go/JS 等源代码文件

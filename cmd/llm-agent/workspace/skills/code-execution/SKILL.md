...
5. **ExecuteCode 失败必须修正重试**：当 ExecuteCode 返回 Python syntax error 或运行时错误时，**必须分析错误原因、修正 Python 代码后再次调用 ExecuteCode**。严禁因为代码报错就放弃沙箱执行，转而逐个直接调用工具或用其他方式绕过。沙箱是数据处理的正确路径，代码错误只需要修复代码本身

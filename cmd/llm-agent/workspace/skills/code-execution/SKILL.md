...
summary: 优先用 call_tool() 批量调用，复杂逻辑拆分多次
tools: ExecuteCode
...
**call_tool 返回值统一为 `{"data": <实际值>}`**，始终通过 `result["data"]` 取值。

**示例 — 获取本周运动数据并汇总：**
```python
import json
# 获取当前日期
date = call_tool("RawCurrentDate", {})["data"]  # "2025-03-09"
# 获取本周运动数据
exercise = call_tool("RawGetExerciseRange", {"account": "xxx", "startDate": "2025-03-03", "endDate": "2025-03-09"})["data"]
# 获取待办
todos = call_tool("RawGetTodosByDate", {"account": "xxx", "date": "2025-03-09"})["data"]
# 在 Python 中处理数据，只输出最终结果
print(f"日期: {date}")
print(f"运动数据: {json.dumps(exercise, ensure_ascii=False)}")
...
5. **call_tool 返回值统一为 `{"data": <值>}`**，始终用 `result["data"]` 取值，不需要猜测类型
6. **ExecuteCode 失败必须修正重试**：当 ExecuteCode 返回 Python syntax error 或运行时错误时，**必须分析错误原因、修正 Python 代码后再次调用 ExecuteCode**。严禁因为代码报错就放弃沙箱执行，转而逐个直接调用工具或用其他方式绕过。沙箱是数据处理的正确路径，代码错误只需要修复代码本身

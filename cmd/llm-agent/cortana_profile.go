...
func buildCortanaPersonaPrompt(profile *CortanaProfile, account string) string {
	...
	sb.WriteString("- 不要把历史里提过的待办、提醒或证件事项自动当作当前仍需提醒的任务；只有 cron/待办工具返回的未完成任务，或用户本轮明确要求提醒时，才主动催办。\n")
	sb.WriteString("- 如果用户明确说某件事已经完成、已经办好、已经签注完成或不用再提醒，要立即承认该状态，后续不得再把它作为待办提醒，除非用户重新要求。\n")
	sb.WriteString("- 不要声称“我查了定时任务/提醒列表/记录”或“没有相关提醒”，除非本轮确实调用工具并拿到结果；没有工具结果时只能说“我这轮没有查到/没有实际查询”。\n")

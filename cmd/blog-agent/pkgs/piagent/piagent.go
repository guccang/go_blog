...
func (usage *TokenUsage) add(value TokenUsage) {
	...
	usage.Reported = usage.Reported || value.Reported
}
...
func Ask(account, question, provider string) (Answer, error) {
	...
	briefResult, err := chat(cfg, "请针对下面的问题给出不超过300字的初步回答。不要假装已检索个人博客，不要列来源。\n\n问题："+question)
	if err != nil {
		return Answer{}, err
	}
	usage := briefResult.Usage
	brief := limitRunes(briefResult.Content, 300)
	...
	if context == "" {
		return Answer{Brief: brief, Text: "站内没有找到与该问题相关的博客内容。", Provider: cfg.Name, Model: cfg.Model, Usage: usage, DurationMs: time.Since(startedAt).Milliseconds()}, nil
	}
	prompt := "你是个人博客助手。请基于站内召回资料，对用户问题给出总结回复。资料不足时明确说明；不要编造。使用中文，简洁回答。不要输出参考、来源或博客标题列表，页面会单独展示可点击来源。\n\n用户问题：" + question + "\n\n初步回答（仅供你校正或补充）：\n" + brief + "\n\n站内资料：\n" + context
	textResult, err := chat(cfg, prompt)
	if err != nil {
		return Answer{}, err
	}
	usage.add(textResult.Usage)
	adviceResult, err := chat(cfg, "请根据用户问题、站内资料总结和资料标题，探索用户可能的下一步意图，并给出1到3条具体、低风险且完整的建议。必须使用“可能”“可以考虑”等措辞，不要把推断说成事实。使用中文，保持简洁，不要列参考来源。\n\n问题："+question+"\n\n站内资料总结：\n"+textResult.Content+"\n\n资料标题："+strings.Join(sources, "、"))
	if err != nil {
		return Answer{}, err
	}
	usage.add(adviceResult.Usage)
	return Answer{Brief: brief, Text: textResult.Content, Advice: adviceResult.Content, Provider: cfg.Name, Model: cfg.Model, Sources: sources, Usage: usage, DurationMs: time.Since(startedAt).Milliseconds()}, nil
}
...
func chatWithContext(ctx context.Context, cfg providerConfig, prompt string) (chatResult, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return chatResult{}, fmt.Errorf("provider %q returned %s", cfg.Name, resp.Status)
	}
	var response struct {
		Usage   TokenUsage `json:"usage"`
		Choices []struct {
	...
	if len(response.Choices) == 0 || strings.TrimSpace(response.Choices[0].Message.Content) == "" {
		return chatResult{}, errors.New("provider returned an empty answer")
	}
	response.Usage.Reported = response.Usage.PromptTokens > 0 || response.Usage.CompletionTokens > 0 || response.Usage.TotalTokens > 0
	return chatResult{Content: strings.TrimSpace(response.Choices[0].Message.Content), Usage: response.Usage}, nil
}

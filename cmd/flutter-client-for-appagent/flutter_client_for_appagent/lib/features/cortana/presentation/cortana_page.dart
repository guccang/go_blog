...
import 'package:flutter_client_for_appagent/shared/utils/speech_transcript_formatter.dart';

part 'cortana_models.dart';
part 'cortana_page_web_fallback.dart';
part 'cortana_page_widget.dart';
...
class CortanaPageState extends State<CortanaPage>
  ...
  String? _webViewUnavailableReason;
  ...
  void initState() {
    ...
    _debugStateTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      if (_shouldPollLive2dDebugState) {
        unawaited(_refreshLive2dDebugState());
      }
    });
  ...
  void _ensureAssetLocalhostServer() {
    ...
    try {
      ...
      _webViewUnavailableReason = 'Localhost server unavailable: $error';
      addFlutterClientLog(_webViewUnavailableReason!);
      _androidLocalhostFuture = Future<void>.value();
      return;
    }
  ...
  Future<void> _syncModelViewTransform() async {
    ...
    await _refreshLive2dDebugState(force: true);
  }
  ...
  Future<void> _callJS(String js, {bool logResult = true}) async {
  ...
  bool get _shouldPollLive2dDebugState =>
      _controlPanelVisible && _live2dSummaryExpanded;

  Future<void> _refreshLive2dDebugState({bool force = false}) async {
    if (!force && !_shouldPollLive2dDebugState) {
      return;
    }
    if (!_webViewReady) {
      return;
    ...
    try {
      return InAppWebView(
        ...
        onWebViewCreated: (ctrl) {
          _webCtrl = ctrl;
          _webViewUnavailableReason = null;
        ...
        onLoadStop: (ctrl, url) async {
          ...
          await _callJS(
            'window.setIdleFocus(${jsonEncode(_immersiveUiHidden)})',
          );
          await _refreshLive2dDebugState(force: true);
        },
      ...
      _webViewUnavailableReason = 'WebView unavailable: $error';
      addFlutterClientLog(_webViewUnavailableReason!);
      debugPrint('[Cortana] $_webViewUnavailableReason');
  ...
  void _startLipSyncLoop(String replyText) {
    ...
    void pushLipSync(Duration position) {
      ...
      _callJS(
        'window.startLipSync(${_jsDouble(currentAmplitude)})',
        logResult: false,
      );
    }
  ...
  List<CortanaSuggestedReply> _defaultSuggestedRepliesForText(String text) {
    ...
    if (RegExp(r'(几点|什么时候|大概.*出发|出发.*时间)').hasMatch(normalized)) {
      return const <CortanaSuggestedReply>[
  ...
  Offset _clampFloatingOffset(Offset offset, Size floatingSize) {
    final screenSize = MediaQuery.sizeOf(context);
    final safeScreenWidth = screenSize.width.isFinite
        ? math.max(0.0, screenSize.width)
        : 0.0;
    final safeScreenHeight = screenSize.height.isFinite
        ? math.max(0.0, screenSize.height)
        : 0.0;
    final safeFloatingWidth = floatingSize.width.isFinite
        ? math.max(0.0, floatingSize.width)
        : 0.0;
    final safeFloatingHeight = floatingSize.height.isFinite
        ? math.max(0.0, floatingSize.height)
        : 0.0;
    final topPadding = math.max(
      0.0,
      MediaQuery.paddingOf(context).top + kToolbarHeight,
    );
    final floatingBottomInset = widget.floatingBottomInset.isFinite
        ? widget.floatingBottomInset
        : 0.0;
    final bottomPadding = math.max(
      0.0,
      MediaQuery.paddingOf(context).bottom + 80 + floatingBottomInset,
    );
    final maxDx = math.max(0.0, safeScreenWidth - safeFloatingWidth);
    final maxAvailableDy = math.max(
      0.0,
      safeScreenHeight - safeFloatingHeight - bottomPadding,
    );
    final minDy = math.min(topPadding, maxAvailableDy);
    final fallbackDx = math.min(
      maxDx,
      math.max(0.0, safeScreenWidth - safeFloatingWidth - 12),
    );
    final fallbackDy = math.min(
      maxAvailableDy,
      math.max(
        minDy,
        safeScreenHeight - safeFloatingHeight - bottomPadding + 80,
      ),
    );

    final rawDx = offset.dx.isFinite ? offset.dx : fallbackDx;
    final rawDy = offset.dy.isFinite ? offset.dy : fallbackDy;
    final dx = rawDx.clamp(0.0, maxDx).toDouble();
    final dy = rawDy.clamp(minDy, maxAvailableDy).toDouble();
  ...
  String _shortValue(Object? value) {
    final text = sanitizeWellFormedUtf16((value ?? '').toString()).trim();
    if (text.isEmpty) {
      return '-';
    }
    if (text.length <= 42) {
    ...
    if (_usesCustomLive2dModel) {
      ...
      return FutureBuilder<_CustomCortanaWebRoot>(
        ...
        builder: (context, snapshot) {
          ...
          final url = !kIsWeb && Platform.isAndroid

...
increment_version() {
    ...
    # 生成版本文件供 Flutter 代码使用（使用 display_version）
...
EOF
    echo "  已生成版本文件: $version_file (版本: $display_version)"
}
...
if [ -f "$STANDARD_APK" ]; then
    ...
    CURRENT_VERSION=$(grep "^version:" "$FLUTTER_DIR/pubspec.yaml" | sed 's/version: *//' | tr -d ' ')
    if [[ "$CURRENT_VERSION" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)\+([0-9]+)$ ]]; then

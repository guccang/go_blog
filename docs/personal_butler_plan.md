...
> 实现备注（严格对齐文档需求后）：
> - **P1**：记忆工具 `RawMemoryRead/Write/Append/Journal/Search/ListFiles` + 新增 `RawMemoryRewriteSection`（§4.1 小节级重写，记忆整理用，纯函数 `replaceMarkdownSection` 有单测）。
> - **P3**：goal 数据结构新增**结构化 `judge` 判据字段**（`goal.Goal`/`GoalSummary`），`RawSaveGoal` 支持写入、`RawGetCurrentGoals` 透出；判官 `GOAL_REVIEW_JOB` 改为依据 `judge` 字段评审；对话拆解（BUTLER_RULES 第 2 条）写入 judge 字段。

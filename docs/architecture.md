...
│(deploy-agent)│  UAP WS │   (消息路由中枢)      │  UAP WS │(llm-mcp-agent)  │
└─────────────┘         └──────────┬───────────┘         └─────────────────┘
                                   │ UAP WebSocket
                        ┌──────────▼───────────┐
                        │    Go Blog Server    │
...
**职责**：
- 企业微信 Callback 接收
- 消息路由（命令 → go_blog，自然语言 → llm-mcp-agent）
...
| **UAP 协议** | llm-mcp-agent 任务 | `task_id` |
...
| `cmd/llm-mcp-agent/` | LLM-MCP Agent 全部代码 |
